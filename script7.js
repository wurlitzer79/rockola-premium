        ...
        const cats = ['Todos', ...new Set(movies.map(m => m.cat))];
        cats.forEach(c => {
            const li = document.createElement('li');
            li.className = `categoria-item ${c === 'Todos' ? 'activa' : ''}`;
            li.innerHTML = `<i class="fas fa-compact-disc"></i> <span>${c}</span>`;
            li.onclick = () => {
        ...
        function filtrar(c) {
            document.getElementById('titulo-seccion').innerText = c;
            const filtradas = c === 'Todos' ? movies : movies.filter(m => m.cat === c);
            render(filtradas);
        }

        function render(lista) {
            container.innerHTML = lista.map(p => `
                <div class="tarjeta-pelicula" onclick="play('${p.url}')">
                    <img src="${p.thumb}" class="imagen-pelicula" loading="lazy">
                    <div class="info-pelicula"><div class="titulo-peli">${p.title}</div></div>
                </div>
            `).join('');
        }

        // 2. REPRODUCTOR E INTERFAZ INTELIGENTE
        function play(url) {
            if(url === '#') { alert("Enlace de ejemplo no disponible"); return; }
            video.src = url;
        ...
        function handleUI() {
            reproductor.classList.add('show-controls');
            clearTimeout(controlTimeout);
            
            // Oculta la "X" y los controles tras 3 segundos de no mover el mouse
            controlTimeout = setTimeout(() => {
                if (!video.paused) {
                    reproductor.classList.remove('show-controls');
                }
            }, 3000);
        }

        reproductor.onmousemove = handleUI;
        reproductor.onclick = handleUI;

        document.getElementById('btn-cerrar').onclick = (e) => {
            e.stopPropagation();
            video.pause();
            video.src = "";
        ...
        video.ontimeupdate = () => {
            document.getElementById('progreso').value = (video.currentTime / video.duration) * 100;
        };

        document.getElementById('progreso').oninput = function() {
